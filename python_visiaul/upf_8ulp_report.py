...
def get_test_array(test_array):
    ...
    for child in test_item.children:
        ...
        test.name = child.find('td', 'STRING_cell').text.replace(' ', '').replace('\n', '') \
                .replace('/rtl_upf_rom_default_usb/rtl_upf_rom_default_usb_SYS_BOOT_upf/rtl_upf_rom_default_usb_sys_boot_vectors_set/', '')
...
def get_sys_boot_dict(owner_dict):
    data = xlrd.open_workbook('owner_upf.xlsx')
    table = data.sheet_by_name('SYS_BOOT_upf')
...
def send_mail():
    ...
    mail_msg = open('8ulp_result_upf.html').read()
...
def create_html(test_array):
    ...
    # Create two picture
    html_file = open('8ulp_result_upf.html', 'w+')
