...
static struct mem_block* arena2block(struct arena* a, uint32_t idx) {
	return (struct mem_block*) ((uint32_t) a + sizeof(struct arena) + idx
			* a->desc->block_size);
}
...
void* sys_malloc(uint32_t size) {
	...
	if (size > 1024) {
		...
		/* 开始分配内存块 */
		b
				= elem2entry(struct mem_block, free_elem, list_pop(&(descs[desc_idx].free_list)));
...
static void vaddr_remove(enum pool_flags pf, void* _vaddr, uint32_t pg_cnt) {
	...
	if (pf == PF_KERNEL) { // 内核虚拟内存池
		...
		struct task_struct* cur_thread = running_thread();
		bit_idx_start = (vaddr - cur_thread->userprog_vaddr.vaddr_start)
				/ PG_SIZE;
		while (cnt < pg_cnt) {
...
static void mem_pool_init(uint32_t all_mem) {
	...
	kernel_vaddr.vaddr_bitmap.bits = (void*) (MEM_BITMAP_BASE + kbm_length
			+ ubm_length);
